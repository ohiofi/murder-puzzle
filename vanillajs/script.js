  ...
  cell.addEventListener("click", () => {
    ...
    if (emojis[nextIndex] == "✅" || currentEmoji == "✅") {
      document.querySelectorAll(".gridCell").forEach((otherCell) => {
        if (
          otherCell.getAttribute("data-row") === row ||
          otherCell.getAttribute("data-col") === col
        ) {
...
function getClue() {
  ...
  document.getElementById(
    "output"
  ).innerHTML += `<p>Clue #${game.clueNumber}: ${clue.text}</p>`;
  game.clueNumber++;
}
...
function solvePuzzle() {
  ...
  let result =
    murderer.suspect.toLowerCase() === suspect.toLowerCase() &&
...
function setPuzzleDate() {
  if (
    Math.floor(
      (new Date() - new Date().getTimezoneOffset()) / (60 * 60 * 24 * 1000)
    ) <= Math.ceil(datepicker.valueAsDate / (60 * 60 * 24 * 1000))
  ) {
    document.getElementById("datepicker").value = new Date().toDateInputValue();
    return;
  }
